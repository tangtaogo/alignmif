...
class _grid_encode(Function):
    ...
    def forward(ctx,
        ...
        _backend.grid_encode_forward(inputs, embeddings, offsets, outputs, B, D,
                                     C, L, S, H, dy_dx, gridtype, align_corners,
                                     interpolation)
    ...
    def backward(ctx, grad):
        ...
        _backend.grid_encode_backward(grad, inputs, embeddings, offsets,
                                      grad_embeddings, B, D, C, L, S, H, dy_dx,
...
class GridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_levels):
            ...
            params_in_level = min(self.max_params,
                                  (resolution if align_corners else resolution +
                                   1)**input_dim)  # limit max number
            params_in_level = int(np.ceil(params_in_level / 8) *
                                  8)  # make divisible
    ...
    def forward(self, inputs, bound=1):
        ...
        outputs = grid_encode(inputs, self.embeddings, self.offsets,
                              ...
                              self.align_corners, self.interp_id)
    ...
    def grad_total_variation(self,
        ...
        _backend.grad_total_variation(inputs, self.embeddings,
                                      self.embeddings.grad, self.offsets,
                                      weight, B, D, C, L, S, H,
                                      self.gridtype_id, self.align_corners)
