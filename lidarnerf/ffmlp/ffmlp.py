...
class _ffmlp_forward(Function):
    ...
    def forward(ctx,
        ...
        # allocate output
        outputs = torch.empty(B,
                              output_dim,
                              device=inputs.device,
                              dtype=inputs.dtype)

        if not inference:
            forward_buffer = torch.empty(num_layers,
                                         B,
                                         hidden_dim,
                                         device=inputs.device,
                                         dtype=inputs.dtype)
            _backend.ffmlp_forward(inputs, weights, B, input_dim, output_dim,
                                   hidden_dim, num_layers, activation,
                                   output_activation, forward_buffer, outputs)
            ctx.save_for_backward(inputs, weights, outputs, forward_buffer)
            ctx.dims = (input_dim, output_dim, hidden_dim, num_layers,
            ...
            inference_buffer = torch.empty(B,
                                           hidden_dim,
                                           device=inputs.device,
                                           dtype=inputs.dtype)
            _backend.ffmlp_inference(inputs, weights, B, input_dim, output_dim,
                                     hidden_dim, num_layers, activation,
                                     output_activation, inference_buffer,
                                     outputs)
    ...
    def backward(ctx, grad):
        ...
        if calc_grad_inputs:
            ...
            grad_inputs = torch.zeros(1, device=grad.device,
                                      dtype=grad.dtype)  # dummy

        grad_weights = torch.zeros_like(weights)
        backward_buffer = torch.zeros(num_layers,
                                      B,
                                      hidden_dim,
                                      device=grad.device,
                                      dtype=grad.dtype)
...
class FFMLP(nn.Module):
    ...
    def __init__(self,
        ...
        self.output_activation = convert_activation(
            'none')  # not supported currently
        ...
        self.num_parameters = hidden_dim * (input_dim + hidden_dim *
                                            (num_layers - 1) +
    ...
    def forward(self, inputs):
        ...
        if pad > 0:
            inputs = torch.cat([
                inputs,
                torch.zeros(pad, C, dtype=inputs.dtype, device=inputs.device)
            ],
                               dim=0)

        outputs = ffmlp_forward(inputs, self.weights, self.input_dim,
                                self.padded_output_dim, self.hidden_dim,
                                self.num_layers, self.activation,
                                self.output_activation, not self.training,
