...
def lidar_to_pano_with_intensities_with_bbox_mask(
    ...
    for bbox_local_point in bbox_local:
        ...
        if r >= lidar_H or r < 0 or c >= lidar_W or c < 0:
            ...
            r_min, r_max, c_min, c_max = min(r_min,
                                             r), max(r_max,
                                                     r), min(c_min,
                                                             c), max(c_max, c)
...
def lidar_to_pano(local_points: np.ndarray,
                  lidar_H: int,
                  lidar_W: int,
                  lidar_K=None,
                  beam_inclinations=None,
...
def lidar_to_pano_with_intensities_fpa(
    ...
    for (local_point, dist, local_point_intensity) in zip(
        ...
        if position > z_buffer_len:
            ...
            intensity_z_buffer = list(
                range_view[r, c, 1][1:]) + [local_point_intensity]
            position = position - 1

            sort_index = np.argsort(depth_z_buffer)
            depth_z_buffer = np.insert(
                np.array(depth_z_buffer)[sort_index][:z_buffer_len], 0,
                position)
            intensity_z_buffer = np.insert(
                np.array(intensity_z_buffer)[sort_index][:z_buffer_len], 0,
                position)
...
def parse_z_buffer(novel_pano, lidar_H, lidar_W, threshold=0.2):
    ...
    for i in range(lidar_H):
        for j in range(lidar_W):
            ...
            final_dis = np.average(final_depth_z_buffer,
                                   weights=1 / final_depth_z_buffer)
            ...
            final_intensity = np.average(final_intensity_z_buffer,
                                         weights=1 / final_depth_z_buffer)
