...
class NeRFNetwork(NeRFRenderer):
    ...
    def __init__(self,
        ...
        self.encoder_rgb = tcnn.Encoding(
            n_input_dims=3,
            encoding_config={
                "otype": "HashGrid",
                "n_levels": 16,
        ...
        self.encoder_lidar = tcnn.Encoding(
            n_input_dims=3,
            encoding_config={
                "otype": "HashGrid",
                "n_levels": 16,
        ...
        self.sigma_net_cat = tcnn.Network(
            n_input_dims=self.encoder.n_output_dims * 2 + embedding_dim,
    ...
    def density(
        ...
        **kwargs,
        ...
        x_ori = (x + self.bound) / (2 * self.bound)  # to [0, 1]
        x = self.encoder(x_ori)

        mask = torch.zeros_like(x).to(x.device)
        activate_levels = kwargs['activate_levels']
        if activate_levels > 0:
            mask[..., :(activate_levels * self.n_features_per_level)] = 1
        x_lidar = self.encoder_lidar(x_ori)
        x_rgb = self.encoder_rgb(x_ori)
        if cal_lidar_color:
            x = torch.cat([x + x_lidar, (x + x_rgb) * mask], dim=-1)
        else:
            x = torch.cat([(x + x_lidar) * mask, x + x_rgb], dim=-1)

        h = self.sigma_net_cat(x)
    ...
    def get_params(self, lr):
        ...
        params = [
            {
                'params': self.encoder_lidar.parameters(),
                'lr': lr
            },
            {
                'params': self.encoder_rgb.parameters(),
                'lr': lr
            },
            {
                'params': self.sigma_net_cat.parameters(),
                'lr': lr
            },
            {
            ...
            {
                'params': self.encoder_lidar_dir.parameters(),
                'lr': lr
            },
            {
            ...
            {
                'params': self.lidar_color_net.parameters(),
                'lr': lr
            },
        ]
