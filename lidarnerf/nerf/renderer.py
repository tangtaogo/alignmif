...
from lidarnerf.raymarching import raymarching
...
class NeRFRenderer(nn.Module):
    ...
    def run(self,
        ...
        # sample steps
        if cal_lidar_color:
            nears = torch.ones(N, dtype=rays_o.dtype,
                               device=rays_o.device) * self.min_near_lidar
            fars = torch.ones(
                N, dtype=rays_o.dtype, device=rays_o.device
            ) * self.min_near_lidar * kwargs['lidar_max_depth']  # hard code
        else:
        ...
        #     density_outputs['sigma'] = density_outputs['sigma'].detach().clone()
