...
class LidarNVSNksr(LidarNVSMeshing):
    ...
    def _run_nksr(
        ...
        input_xyz = torch.from_numpy(np.asarray(pcd.points)).float().to(
            self.device)
        input_normal = torch.from_numpy(np.asarray(pcd.normals)).float().to(
            self.device)

        field = self.nksr_reconstructor.reconstruct(input_xyz,
                                                    input_normal,
                                                    detail_level=0.5)
