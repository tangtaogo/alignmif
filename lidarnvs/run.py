...
def main():
    ...
    if args.enable_collect_raydrop_dataset:
        if args.method == 'poisson' and args.dataset != "nerf_mvl":
            raydrop_train_data = generate_raydrop_data_meshing(
                train_dataset, nvs)
            raydrop_test_data = generate_raydrop_data_meshing(test_dataset, nvs)
        elif args.method == 'pcgen':
            raydrop_train_data = generate_raydrop_data_pcgen(train_dataset,
                                                             nvs,
                                                             rm_pano_mask=False)
            raydrop_test_data = generate_raydrop_data_pcgen(test_dataset,
                                                            nvs,
                                                            rm_pano_mask=False)
        else:
            raise ValueError(
                f"Unknown method/dataset: {args.method}/{args.dataset}")

        data_dir = Path(
            "data/raydrop") / args.method / f"{args.dataset}_{args.sequence_id}"
