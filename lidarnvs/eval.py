...
def eval_points_and_pano(
    ...
    is_instance_all = [
        isinstance(e, np.ndarray) for e in [
            gt_local_points, pd_local_points, gt_intensities, pd_intensities,
            gt_pano, pd_pano
        ]
    ]
    ...
    def compute_depth_metrics(gt_depths,
                              pd_depths,
                              min_depth=1e-3,
                              max_depth=80,
                              thresh_set=1.25):
    ...
    (
        ...
        metrics["depth_a3"],
        metrics["depth_ssim"],
    ) = compute_depth_metrics(gt_depths=gt_pano.flatten(),
                              pd_depths=pd_pano.flatten())

    (
        metrics["chamfer"],
        metrics["f_score"],
    ) = compute_point_metrics(gt_points=gt_local_points,
                              pd_points=pd_local_points)
