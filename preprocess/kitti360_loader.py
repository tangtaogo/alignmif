...
class KITTI360Loader:
    ...
    def _load_perspective_intrinsics(intrinsics_path):
        ...
        with open(intrinsics_path, 'r') as fid:
            perspective_dict = {}
            intrinsic_names = [
                'P_rect_00', 'R_rect_00', 'P_rect_01', 'R_rect_01'
            ]
            ...
            for intrinsic in intrinsic_names:
                if intrinsic.startswith('P_rect'):
                    perspective_dict[intrinsic] = np.concatenate(
                        (KITTI360Loader._read_variable(fid, intrinsic, 3,
                                                       4), last_row))
                else:
    ...
    def get_image_paths(self, camera_name, sequence_name, frame_ids):
        ...
        # Get image paths.
        im_dir = self.data_2d_raw_dir / f"{sequence_name}_sync" / subdir_name / "data_rect"
    ...
    def _load_all_cameras(self, sequence_name):
        ...
        perspective_dict = KITTI360Loader._load_perspective_intrinsics(
            perspective_path)
        ...
        for frame_id in frame_ids:
            ...
            cam_00_to_world_dict[frame_id] = ct.convert.pad_0001(
                cam_00_to_world)
        ...
        for frame_id in frame_ids:
            ...
            cam_01_to_world_dict[frame_id] = ct.convert.pad_0001(
                cam_01_to_world)
        ...
        for frame_id in frame_ids:
            ...
            assert np.allclose(gt_cam_00_to_world,
                               cam_00_to_world,
                               atol=1e-5,
                               rtol=1e-5)
    ...
    def _load_all_lidars(self, sequence_name):
        ...
        for frame_id in frame_ids:
            imu_to_world = imu_to_world_dict[frame_id]
            cam_00_to_world_unrec = imu_to_world @ cam_00_to_imu
            velo_to_world = cam_00_to_world_unrec @ np.linalg.inv(
                cam_00_to_velo)
    ...
    def load_lidars(self, sequence_name, frame_ids):
        ...
        velo_to_worlds = [
            velo_to_world_dict[frame_id] for frame_id in frame_ids
        ]
...
def main():
    ...
    # Load images.
    im_cam_00s = k3.load_images(camera_name="cam_00",
                                sequence_name="2013_05_28_drive_0000",
                                frame_ids=range(1908, 1971 + 1))
    im_cam_01s = k3.load_images(camera_name="cam_01",
                                sequence_name="2013_05_28_drive_0000",
                                frame_ids=range(1908, 1971 + 1))

    # Visualize.
    cam_00_frames = ct.camera.create_camera_ray_frames(cam_00_Ks,
                                                       cam_00_Ts,
                                                       size=0.8)
    cam_01_frames = ct.camera.create_camera_ray_frames(cam_01_Ks,
                                                       cam_01_Ts,
                                                       size=0.8)
